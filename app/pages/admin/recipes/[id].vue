...
async function handleGenerateSkeletalLogic() {
  ...
  try {
    const result = await $fetch<SkeletalLogicAnalysis>('/api/admin/generate-skeletal-logic', {
      method: 'POST',
      body: {
        transcript,
...
function togglePremium() {
  if (!video.value) return
  video.value.is_premium = !video.value.is_premium
  scheduleAutoSave()
}
...
function handleSelectVideo(selectedVideo: Video) {
  if (selectedVideo.id !== videoId) {
    ...
    router.push(`/admin/recipes/${selectedVideo.id}`)
  }
}
    ...
    >
      <div class="flex items-center gap-3">
        ...
        <UBadge
          ...
          :variant="video.is_premium ? 'solid' : 'outline'"
          size="xs"
          class="cursor-pointer"
          @click="togglePremium"
        >
          <UIcon name="i-ph-crown-simple" class="w-3 h-3 mr-1" />
